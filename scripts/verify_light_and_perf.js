...
(async () => {
  ...
  console.log('HTML Theme verification:', JSON.stringify(htmlTheme));

  // Capture 01_dashboard_light.png
  const filePath = path.join(process.cwd(), 'audit_proof', '01_dashboard_light.png');
  await page.screenshot({ path: filePath, fullPage: false });
  console.log(`Saved: ${filePath}`);
  fs.copyFileSync(filePath, path.join(artifactDir, '01_dashboard_light.png'));

  await browser.close();
  console.log('✅ Light theme & dashboard screenshot complete.');
})();

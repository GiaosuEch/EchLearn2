...
### 6-Part Master Prompt Architecture
When turning an initial brief into a production-grade prompt, output this 6-part framework:
1. **Role**: Specific expert identity & authority.
2. **Context**: Background, motivation, and target audience.
3. **Task**: Explicit, actionable deliverables.
4. **Constraints**: Negative boundaries, style rules, and anti-slop guidelines.
5. **Chain of Thought**: Required step-by-step reasoning phase before generating content.
6. **Output Format**: Structural schema, markdown headings, tables, or JSON.

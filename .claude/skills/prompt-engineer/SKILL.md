...
### 6-Part Master Prompt Standard Architecture

When asked to turn a brief user task into an optimal, production-grade prompt, ALWAYS structure the generated prompt in a copy-pasteable Markdown block using this 6-part framework:

1. **Role (Vai trò chuyên gia)**: Specific expert persona, domain credentials, and baseline tone.
2. **Context (Bối cảnh & Đối tượng)**: Background situation, problem space, and target audience.
3. **Task (Nhiệm vụ cốt lõi)**: Unambiguous, actionable, and definitive instruction of what to produce.
4. **Constraints (Quy tắc & Giới hạn cấm kỵ)**: Explicit negative constraints, banned phrases, length limits, and anti-slop rules.
5. **Chain of Thought (Tư duy từng bước)**: Mandatory reasoning / outline steps the AI must execute before final output.
6. **Output Format (Định dạng đầu ra)**: Precise layout requirements (Markdown headers, tables, callouts, JSON schema).

  ...
  return (
    <header className="ech-topbar h-16 flex items-center justify-between px-4 lg:px-8 bg-white dark:bg-slate-950 text-slate-900 dark:text-white relative z-[70] border-b border-slate-200 dark:border-slate-800">
      ...
      <div className="flex items-center gap-2">
        ...
        <TopBarStats />
        <button id="theme-toggle-topbar" type="button" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="p-3 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200" title="Chuyển giao diện sáng/tối">{theme === 'dark' ? <Sun size={18} /> : <Moon size={18} />}</button>
        <div className="relative" ref={notifRef}>
          <button id="notifications-menu-button" ref={notificationsTriggerRef} type="button" onClick={() => setShowNotifications(value => !value)} aria-expanded={showNotifications} aria-controls="notifications-popover" className="p-3 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200" aria-label="Thông báo"><Bell size={18} /></button>
        ...
        <button type="button" onClick={() => setShowAccountSwitcher(true)} className="flex items-center gap-2 p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"><div className="w-9 h-9 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold text-xs">{displayName?.[0] || 'U'}</div><span className="hidden md:inline text-xs font-semibold max-w-[100px] truncate">{displayName || 'Bạn'}</span><ProBadge className="hidden sm:inline-flex" /></button>

...
import { DeepArticleReader } from '../../../components/immersion/DeepArticleReader';
import type { DeepArticle } from '../../../curriculum/contentTypes';
import { Loader2, AlertCircle, ChevronLeft, ChevronRight } from 'lucide-react';

export default function ImmersionArticlePage() {
  const currentLanguage = useAppStore((state) => state.currentLanguage);
  const [articles, setArticles] = useState<DeepArticle[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  
  // Basic pagination state
  const [page, setPage] = useState(0);
  const limit = 1; // Show one article at a time for deep reading

  const getArticles = useContentCacheStore((state) => state.getArticles);
  ...
  useEffect(() => {
    ...
    const fetchArticles = async () => {
      try {
        ...
        const cachedEntry = useContentCacheStore.getState().articles[cacheKey];
        ...
        const data = await getArticles(currentLanguage, limit, page * limit);
  ...
  }, [currentLanguage, page, getArticles]);

  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh]">
        <Loader2 className="w-12 h-12 text-primary animate-spin mb-4" />
        <p className="text-slate-500 font-medium">Loading high-density academic content...</p>
      </div>
    );
  }
  ...
  if (!articles || articles.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center px-4">
        <h2 className="text-2xl font-bold text-slate-800">No Deep Articles Available</h2>
        <p className="text-slate-500 mt-2 max-w-md">The Content Engine has not generated any articles for language '{currentLanguage}' on this page.</p>
        {page > 0 && (
          <button 
  ...
  return (
    <div className="pb-20">
      <DeepArticleReader article={articles[0]} />
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 flex justify-between items-center border-t border-slate-100 mt-8">
        ...
        <div className="text-slate-400 font-medium">Page {page + 1}</div>

    ...
    >
      <div className="max-w-4xl mx-auto space-y-6 font-mono">
        ...
        <div className="glass-card p-6 border-2 border-emerald-500/30 bg-white dark:bg-slate-950 rounded-3xl shadow-xl space-y-6">
          <div className="flex flex-col md:flex-row gap-6 items-center border-b border-slate-200 dark:border-slate-800 pb-4">
            <img src={selectedArticle.coverImage} alt={selectedArticle.titleVi} className="w-32 h-32 rounded-2xl object-cover border border-slate-200 dark:border-slate-800" />
            <div className="flex-1 space-y-2 text-center md:text-left">
              <div className="flex items-center justify-center md:justify-start gap-2">
                <span className="px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 text-xs font-bold border border-emerald-500/30 flex items-center gap-1.5">
                  <Newspaper size={14} /> {selectedArticle.category}
                </span>
                <span className="px-2.5 py-0.5 rounded-md bg-amber-500/15 dark:bg-slate-800 text-amber-700 dark:text-amber-300 text-[11px] font-bold border border-amber-500/20 dark:border-slate-700">
                  Trình độ: {selectedArticle.level}
                </span>
                <span className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1"><Clock size={12} /> {selectedArticle.readTime}</span>
              </div>
              <h2 className="text-xl font-black text-slate-900 dark:text-white">{selectedArticle.titleVi}</h2>
              <p className="text-xs text-emerald-600 dark:text-emerald-400 italic">"{selectedArticle.titleTarget}"</p>
            </div>
            <button
              onClick={() => setShowViTranslation(!showViTranslation)}
              className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-white transition-colors cursor-pointer"
            >
          ...
          <div className="space-y-6">
            {selectedArticle.paragraphs.map((p, idx) => (
              <div key={idx} className="p-5 rounded-2xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl bg-slate-100 hover:bg-emerald-600 hover:text-white text-slate-600 dark:bg-slate-800 dark:hover:bg-emerald-500 dark:text-slate-300 transition-all cursor-pointer"
                ...
                {showViTranslation && (
                  <p className="text-sm text-slate-600 dark:text-slate-400 pl-4 border-l-2 border-emerald-500 italic leading-relaxed">
                    {p.vi}
                  </p>
                )}
              </div>

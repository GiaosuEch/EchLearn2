...
export default function PracticeHubPage() {
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-7 pb-20">
      <header className="rounded-2xl border border-slate-200 bg-slate-50 p-6 dark:border-slate-800 dark:bg-slate-900 sm:p-8">
        <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Trung tâm luyện tập</p>
        <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-900 dark:text-white">Chọn một việc bạn muốn làm tốt hơn</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-400">Mỗi mục bên dưới nói rõ bạn sẽ học gì, hành động chính và kết quả cần đạt. Bắt đầu với một mục tiêu duy nhất.</p>
      </header>
        ...
        >
          {visibleCards.map((card) => (
            <motion.article 
              ...
              className="flex flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-none transition-shadow hover:shadow-[0_2px_8px_rgba(0,0,0,0.04)] dark:border-slate-800 dark:bg-slate-900"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex size-12 items-center justify-center rounded-xl bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300">{card.icon}</div>
                <span className="inline-flex items-center gap-1 text-[11px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500"><Clock size={12} /> {card.duration}</span>
              </div>
              ...
              <p className="mt-4 rounded-xl bg-slate-50 p-4 text-sm font-semibold leading-6 text-slate-700 dark:bg-slate-800/50 dark:text-slate-300">{card.outcome}</p>
              <Link to={card.path} className="mt-6 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-3 text-sm font-bold text-white transition-colors hover:bg-slate-800 active:scale-[0.98] dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100">
